...
The LSP must be installed separately from Neovim and reachable at the path expected by (or configured via) that plugin.
...
- `gra`: run the Code Action available at that line, or open a menu to select if many are available
...
- `grx`: run the Codelens available at that line, or open a menu to select if many are available

### Navigation

- `space-sf`: search for files by name with a fuzzy finder
- `space-sg`: search file contents with a fuzzy finder
- `space-sw`: search for occurrences of the word under the cursor
